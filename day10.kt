fun main() {
    ...
    var wayIn = 1
    var length = 0
    val map = day10input.split("\n").toTypedArray()

    fun updatePos(pos : Pair<Int,Int>, way : Int) {
        currentLocation = pos
        wayIn = way
    }

    // We came in from 0 - West (or start), 1 - East - 2 North - 3 South
    fun stepToNext() {
        map[currentLocation.first][currentLocation.second].let { mapToken ->
            length++
            when (mapToken) {
                '-' -> // horizontal pipe connecting east (1) and west (0)
                    when (wayIn) {
                        0 -> updatePos(Pair(currentLocation.first, currentLocation.second - 1),0)
                        1 -> updatePos(Pair(currentLocation.first, currentLocation.second + 1),1)
                        else -> throw Exception("bad stuff happened 1")
                    }
                '|' -> // vertical pipe connecting north (2) and south (3).
                    when (wayIn) {
                        2 -> updatePos(Pair(currentLocation.first-1, currentLocation.second), 2)
                        3 -> updatePos(Pair(currentLocation.first+1, currentLocation.second), 3)
                        else -> throw Exception("bad stuff happened 2")
                    }
                'L' -> // 90-degree bend connecting north (2) and east (1).
                    when (wayIn) {
                        3 -> updatePos(Pair(currentLocation.first, currentLocation.second + 1), 1)
                        0 -> updatePos(Pair(currentLocation.first-1, currentLocation.second),2)
                        else -> throw Exception("bad stuff happened 3")
                    }
                'J' -> //  90-degree bend connecting north (2) and west (0).
                    when (wayIn) {
                        3 -> updatePos(Pair(currentLocation.first, currentLocation.second - 1), 0)
                        1 -> updatePos(Pair(currentLocation.first-1, currentLocation.second),2)
                        else -> throw Exception("bad stuff happened 4")
                    }
                '7' -> // 90-degree bend connecting south (3) and west (0) .
                    when (wayIn) {
                        1 -> updatePos(Pair(currentLocation.first+1, currentLocation.second), 3)
                        2 -> updatePos(Pair(currentLocation.first, currentLocation.second-1), 0)
                        else -> throw Exception("bad stuff happened 5")
                    }
                'F' -> // 90-degree bend connecting south (3) and east (1).
                    when (wayIn) {
                        0 -> updatePos(Pair(currentLocation.first+1, currentLocation.second), 3)
                        2 -> updatePos(Pair(currentLocation.first, currentLocation.second+1), 1)
                        else -> throw Exception("bad stuff happened 6")
                    }
                'S' -> updatePos(Pair(0,0),0)
                else -> throw Exception("bad stuff happened 7")
            }
    ...
    currentLocation = map.find {
            ...
            Pair(map.indexOf(line),line!!.indexOf("S")+1)
        }

    do {
        stepToNext()
    } while (currentLocation.first != 0 && currentLocation.second != 0)
