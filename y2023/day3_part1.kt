...
fun main() {
    val matrix = grid.split("\n").toTypedArray()
    var num = ""
    var keep = false
    var sum = 0

    fun validate(line: Int, index: Int): Boolean {
        if (keep) return true
        return !(matrix[line].toCharArray()[index].isDigit() || matrix[line].toCharArray()[index] == '.')
    }
    (0..matrix.size - 1).forEach { line ->
        matrix[line].forEachIndexed { index, it ->
            if (it.isDigit()) {
                num += it.digitToInt()
                if (line > 1 && index > 1) keep = validate(line - 1, index-1)
                if (line > 1) keep = validate(line - 1, index)
                if (line > 1 && index < matrix[line].length - 1) keep = validate(line - 1, index+1)

                if (index > 1) keep = validate(line, index - 1)
                if (index < matrix[line].length - 1) keep = validate(line, index+1)

                if (line + 1 < matrix.size && index > 1) keep = validate(line + 1, index - 1)
                if (line + 1 < matrix.size) keep = validate(line + 1, index)
                if (line + 1 < matrix.size && index < matrix[line].length - 1) keep = validate(line + 1, index + 1)
            } else {
