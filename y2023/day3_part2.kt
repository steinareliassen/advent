...
fun main() {
    ...
    fun validate(line: Int, index: Int): Pair<Int, Int>? =
        if (pair != null) pair
        else if(matrix[line].toCharArray()[index] == '*') Pair(line, index) else null

    (0..matrix.size - 1).forEach { line ->
        matrix[line].forEachIndexed { index, it ->
            if (it.isDigit()) {
                num += it.digitToInt()
                if (line > 1 && index > 1) pair = validate(line - 1, index-1)
                if (line > 1) pair = validate(line - 1, index)
                if (line > 1 && index < matrix[line].length - 1) pair = validate(line - 1, index+1)

                if (index > 1) pair = validate(line, index - 1)
                if (index < matrix[line].length - 1) pair = validate(line, index+1)

                if (line + 1 < matrix.size && index > 1) pair = validate(line + 1, index - 1)
                if (line + 1 < matrix.size) pair = validate(line + 1, index)
                if (line + 1 < matrix.size && index < matrix[line].length - 1) pair = validate(line + 1, index + 1)
            } else {
                val actualNum = if (num.isBlank()) 0 else num.toInt()
                if (actualNum != 0 && pair != null) {
                    if (pairs.containsKey(pair)) pairs[pair]?.add(actualNum)
                    else pairs[pair!!] = mutableListOf(actualNum)
                }
                num = ""
                pair = null
            }
